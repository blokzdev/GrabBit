  ...
  final container = ProviderContainer(
    overrides: [
      ...
      settingsControllerProvider.overrideWith(
        () => _FixedSettings(
          SettingsModel(selectedTranscriptionModelId: selectedId),
        ),
      ),
    ],
  );
...
void main() {
  test(
    'low tier still has an active model (whisper-tiny, never gated off)',
    () async {
      final container = _container(tier: DeviceTier.low, selectedId: '');
      await container.read(settingsControllerProvider.future);
      expect(container.read(activeTranscriptionModelProvider), whisperTiny);
      // The engine is the graceful no-op fallback until the native engine (P12e-2).
      expect(container.read(transcriptionEngineProvider).isAvailable, isFalse);
    },
  );
  ...
  test('an empty selection uses the tier recommendation', () async {
    ...
    await container.read(settingsControllerProvider.future);
    expect(container.read(activeTranscriptionModelProvider), whisperBase);
  });

  test('an eligible selection wins over the recommendation', () async {
    final container = _container(
      tier: DeviceTier.high,
      selectedId: whisperLargeV3Turbo.id,
    );
    await container.read(settingsControllerProvider.future);
    expect(
      container.read(activeTranscriptionModelProvider),
      whisperLargeV3Turbo,
    );
  });
  ...
  test(
    ...
    () async {
      // The flagship turbo isn't offered on mid tier → fall back to the mid rec.
      final container = _container(
        tier: DeviceTier.mid,
        selectedId: whisperLargeV3Turbo.id,
      );
  ...
  test('transcriptionEngine is never null and reports its model', () async {
    final container = _container(tier: DeviceTier.low, selectedId: '');
    await container.read(settingsControllerProvider.future);
    final engine = container.read(transcriptionEngineProvider);
    // No native runtime on the CI host → Unavailable, but model still reported.
    expect(engine.isAvailable, isFalse);
    expect(engine.model, whisperTiny);
  });
}
