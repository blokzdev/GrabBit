...
void main() {
  ...
  test('opens at schema version 16 with all tables created', () async {
  ...
  test(
    'upgrades a v13 database to v14, adding the chat tables (P13d-2a)',
    () async {
      // Seed a minimal v13 DB (media_tags already has `source`; no chat tables),
      // user_version=13. Opening at v14 must create chats + chat_messages.
      final upgraded = AppDatabase(
        NativeDatabase.memory(
          setup: (raw) {
            raw.execute('''
      ...
      // The new tables exist and a chat + its messages round-trip.
      const chatId = 'chat_1';
      await upgraded
          .into(upgraded.chats)
          .insert(
      ...
      await upgraded
          ...
          .insert(
            ChatMessagesCompanion.insert(
              ...
              role: 'assistant',
              content: 'hello [1]',
              citationsJson: const Value('[{"i":1,"id":"x","title":"X"}]'),
              createdAt: DateTime.utc(2026),
            ),
          );
      expect(await upgraded.select(upgraded.chatMessages).get(), hasLength(2));

      // Deleting the chat cascades to its messages (foreign_keys = ON).
  ...
  test(
    'upgrades a v15 database to v16, adding the thing_suggestions table (P15c)',
    () async {
      // Seed a minimal v15 DB (the tables the migration tail touches), then open
      // at v16 — the from<16 step must create thing_suggestions.
      final upgraded = AppDatabase(
        NativeDatabase.memory(
          setup: (raw) {
            raw.execute('''
      ...
      // The new table exists and a pending suggestion round-trips.
      await upgraded
          .into(upgraded.thingSuggestions)
          .insert(
            ThingSuggestionsCompanion.insert(
              id: 'sug_1',
              sourceItemId: 'item-1',
              type: 'Recipe',
              jsonld: '{"@type":"Recipe","name":"Carbonara"}',
              confidence: const Value(0.8),
              createdAt: DateTime.utc(2026),
            ),
          );
      final suggestions = await upgraded
          .select(upgraded.thingSuggestions)
          .get();
      expect(suggestions, hasLength(1));
      expect(suggestions.single.sourceItemId, 'item-1');
      expect(suggestions.single.type, 'Recipe');
      expect(suggestions.single.confidence, 0.8);
    },
  );
