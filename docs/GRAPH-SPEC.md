...
| **Path / bridge discovery** (P13) | shortest-path / connectivity between two items or entities. **Realized P13e-3a** for **item↔item** via pure-Dart **BFS** over the bipartite entity graph (`path_finding.dart`) — every-device, no embedder; surfaced as the **"How are these related?"** connection chain, **and in the graph view** as a highlighted **path mode** (P13e-3b). Item↔entity paths + graph-theoretic bridges/articulation points deferred (BACKLOG). |
