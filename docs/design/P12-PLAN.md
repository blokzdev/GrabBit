...
### `[x]` P12e — Whisper transcription (whisper.cpp) *(native — needs an APK build)*
...
- **`[x]` P12e-3** — wire whisper as the caption-less fallback into the existing transcript flows:
  **manual** "Get transcript" is a 3-state on-ramp (set up / download / transcribe; can enable
  transcription itself); **auto-on-download** transcribes caption-less items when a model is already
  present, else posts a one-time "finish setup" nudge. Feeds `MediaMetadata.transcript`/`transcriptCues`.
