...
| **Graph+vector (P10):** `io.github.cozodb:cozo_android:0.7.2` (Maven Central AAR) via a `CozoHostApi` Pigeon bridge; `ffi`+`ffigen` (dev) for the Windows `dart:ffi` impl (P17). MPL-2.0. | On-device graph + HNSW vector DB. See `docs/GRAPH-SPEC.md`. |
| **On-device AI (P10b-2+):** `flutter_gemma` `^0.16.5` (MediaPipe/LiteRT-LM — embeddings + LLM + RAG + **function-calling**; added P10b-2 embedder-only, bumped to 0.16.5 in P15a for Gemma 4 E2B function-calling), a whisper.cpp pkg (`whisper_ggml_plus`/`whisper_kit`), ML Kit (OCR/translate) | On-device/edge AI. See `docs/AI-SPEC.md`. |
...
| **On-device AI (P12e):** `whisper_ggml_plus` (whisper.cpp FFI, MIT) | Speech transcription. Chosen over `whisper_kit` for Windows/P17 parity; app-managed ggml model fed as `modelPath`, 16 kHz WAV made with our existing ffmpeg (no ffmpeg companion). |
...
- **Windows (P17):** bundle `yt-dlp.exe` + `ffmpeg.exe` + `cozo_c.dll` in install dir (the Cozo
